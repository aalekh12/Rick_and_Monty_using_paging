package com.modern.rickandmonty.paging
...
class CharacterPagingSource(val apiService: RetroServices): PagingSource<Int, CharacterData>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CharacterData> {
        return try {
            ...
            if (response?.info?.next != null) {
                ...
                val nextPageQuery = uri.getQueryParameter("character")
                nextPageNumber = nextPageQuery?.toInt()
            }
            var prevPageNumber: Int? = null
            if (response?.info?.prev != null) {
                val uri = Uri.parse(response?.info?.prev!!)
                val prevPageQuery = uri.getQueryParameter("character")
